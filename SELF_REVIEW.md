...
Rules-классификатор и keyword retrieval: хорошо работают на «прямых» формулировках (пароль/списание), ломаются на перефразах (кейс `paraphrase_access` в smoke = LIMIT). Confidence эвристический, не калиброванный. Draft — mock, не проверяет галлюцинации реального LLM.
...
- Один основной язык демо — RU; мультиязык не моделировали.
